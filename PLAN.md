...
- [x] Publicar `v0.1.0-alpha.6` para corregir la ruta `//?/C:/...` observada en `root` de Nginx y pasada como `SCRIPT_FILENAME` a PHP; run manual `35171895408` verde en Linux/Windows y publicación. Falta confirmar que `demo.test/index.php` responda correctamente en la VM Windows.
- [x] Publicar la prerelease `v0.1.0-alpha.7` con Apache opcional por sitio, HTTPS local, recarga automática de Nginx y módulos PHP para frameworks; run manual `35176508736` verde en Linux y Windows. Los paquetes TAR/ZIP y `SHA256SUMS.txt` se descargaron del Release y ambos hashes coinciden. Falta probar la nueva versión en VM Windows.
