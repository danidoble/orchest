...
apt-get update
apt-get install -y --no-install-recommends build-essential ca-certificates curl libpcre2-dev zlib1g-dev libssl-dev patchelf
mkdir -p /build /out
cd /build
...
for library in $(ldd "$package/sbin/nginx" | awk '/libpcre2|libcrypt|libssl|libcrypto/ { print $3 }'); do
